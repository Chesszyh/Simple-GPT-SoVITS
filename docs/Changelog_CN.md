...
- 突然有点质疑自己在做什么……
  - 你把人家webui移除了干嘛，有gnu不用非得退化到headless服务器是吧，而且服务器也可以开端口转发
  - 我能想到的训练+推理完全Headless的好处，就是可以让AI自己训练自己
  - **让AI训练AI，应该是一件很酷的事情吧**
  - 又该学学什么MCP协议了
  - 两条路：
    - 完全移除图形界面，只保留后端供AI调用
      - 强依赖于LLM能力
    - 保留原项目的所有GUI，利用微软Omni-Parser之类的仓库，让AI识别icon然后操作电脑
      - 可能比上面的还难……

- 想在wsl2里安装环境，但是conda用不了，尝试venv也报错
